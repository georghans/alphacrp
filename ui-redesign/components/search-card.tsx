...
export function SearchCard({ search, onDelete }: SearchCardProps) {
  const { toggleActive } = useSearchStore()

  const handleToggle = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    toggleActive(search.id)
  }
          ...
          <div className="absolute left-0 top-0 border-b-2 border-r-2 border-foreground bg-background px-3 py-1">
            <div className="flex items-center gap-2">
              <Circle className={cn(
                ...
                search.isActive ? "text-foreground" : "text-muted-foreground"
              )} />
          ...
          <div className="absolute right-2 top-2 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
            <DropdownMenu>
              <DropdownMenuTrigger asChild onClick={(e) => e.preventDefault()}>
                <Button variant="outline" size="icon" className="h-8 w-8 border-2 border-foreground bg-background hover:border-accent hover:bg-accent hover:text-accent-foreground">
                  <MoreHorizontal className="h-4 w-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="border-2 border-foreground">
                <DropdownMenuItem asChild>
                  <Link href={`/searches/${search.id}`} className="flex items-center gap-2 font-mono text-xs uppercase">
