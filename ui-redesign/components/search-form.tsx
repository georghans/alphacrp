...
export function SearchForm({ search, mode, createId }: SearchFormProps) {
  ...
  const { addSearch, updateSearch, deleteSearch } = useSearchStore()
  
  const [title, setTitle] = useState(search?.title ?? "")
  const [prompt, setPrompt] = useState(search?.prompt ?? "")
  const [searchTermsInput, setSearchTermsInput] = useState(search?.searchTerms?.join(", ") ?? "")
  ...
  const parseSearchTerms = (input: string): string[] => {
    return input.split(",").map(term => term.trim()).filter(term => term.length > 0)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "create") {
      addSearch({
        title: title.trim(),
        prompt: prompt.trim(),
        searchTerms: parseSearchTerms(searchTermsInput),
        images,
        isActive,
      }, createId)
    } else if (search) {
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" asChild className="border-2 border-foreground bg-transparent hover:border-accent hover:bg-accent hover:text-accent-foreground">
            <Link href="/searches">
          ...
          {mode === "edit" && search && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button type="button" variant="outline" className="gap-2 border-2 border-destructive bg-transparent font-mono text-xs uppercase tracking-wider text-destructive hover:bg-destructive hover:text-destructive-foreground">
              ...
              <AlertDialogContent className="border-2 border-foreground">
                <AlertDialogHeader>
                  <AlertDialogTitle className="font-mono text-lg uppercase tracking-wider">DELETE SEARCH</AlertDialogTitle>
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel className="border-2 border-foreground bg-transparent font-mono text-xs uppercase hover:border-accent hover:bg-accent hover:text-accent-foreground">Cancel</AlertDialogCancel>
                  <AlertDialogAction onClick={handleDelete} className="border-2 border-destructive bg-destructive font-mono text-xs uppercase text-destructive-foreground hover:bg-destructive/90">
                    Delete
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-8 lg:col-span-2">
          <div className="border-2 border-foreground bg-card">
            <div className="border-b-2 border-foreground px-6 py-4">
              <h2 className="font-mono text-sm font-bold uppercase tracking-wider text-foreground">SEARCH DETAILS</h2>
            ...
            <div className="space-y-6 p-6">
              <div className="space-y-2">
                <Label htmlFor="title" className="font-mono text-xs uppercase tracking-wider text-foreground">TITLE</Label>
                <Input
                  id="title"
              ...
              <div className="space-y-2">
                <Label htmlFor="prompt" className="font-mono text-xs uppercase tracking-wider text-foreground">STYLE DESCRIPTION</Label>
                <Textarea
                  id="prompt"
              ...
              <div className="space-y-2">
                <Label htmlFor="searchTerms" className="font-mono text-xs uppercase tracking-wider text-foreground">SEARCH TERMS</Label>
                <Textarea
                  id="searchTerms"
          ...
          <div className="border-2 border-foreground bg-card">
            <div className="border-b-2 border-foreground px-6 py-4">
              <h2 className="font-mono text-sm font-bold uppercase tracking-wider text-foreground">REFERENCE IMAGES</h2>
              <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
        ...
        <div className="space-y-8">
          <div className="border-2 border-foreground bg-card">
            <div className="border-b-2 border-foreground px-6 py-4">
              <h2 className="font-mono text-sm font-bold uppercase tracking-wider text-foreground">STATUS</h2>
              <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
            ...
            <div className="border-2 border-foreground bg-card">
              ...
              <div className="space-y-4 p-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-xs text-foreground">{search.createdAt.toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">UPDATED</span>
                  <span className="font-mono text-xs text-foreground">{search.updatedAt.toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between">
