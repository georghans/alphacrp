...
export async function crawlSearch(
  ...
  // Primary path: direct Algolia API (no browser needed)
  try {
    const algoliaOffers = await crawlWithAlgoliaApi(http, config, term, maxPages, maxItems);
    if (algoliaOffers.length > 0) {
      logger.info({ count: algoliaOffers.length }, "Algolia API search succeeded");
      return algoliaOffers;
    }
  } catch (error) {
    logger.warn({ error }, "Algolia API search failed, falling back");
  }
